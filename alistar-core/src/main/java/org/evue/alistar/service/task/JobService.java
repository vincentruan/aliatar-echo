...
import org.evue.alistar.bean.exception.ApplicationExceptionEnum;
import org.evue.alistar.bean.vo.QuartzJob;
    ...
    public QuartzJob getJob(Task task) {
        ...
        if (task != null) {
            ...
            if (StringUtils.isNotBlank(task.getData())) {
                try {
                    Map<String, Object> dataMap = JSON.parseObject(task.getData(), new TypeReference<Map<String, Object>>(){});
                    job.setDataMap(dataMap);
                } catch (Exception e) {
                    logger.error(ApplicationExceptionEnum.TASK_CONFIG_ERROR.getMessage() + "->" + task, e);
